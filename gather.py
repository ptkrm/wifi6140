...
#-c use current wireless network
#-p use proxy
parser = argparse.ArgumentParser(description='Free and Public Hotspot information gather!')
parser.add_argument('-m',help="Enable or Disable MAC Change - default 0",type=int,choices=[0,1],metavar='--mac', required=True, default=0,dest='mac')
parser.add_argument('-c',help="Use Current Wireless Network - default 0",type=int,choices=[0,1],metavar='--current', default=0, dest='current')
parser.add_argument('-p',help="Use Proxy - default 1",type=int,choices=[0,1],metavar='--proxy', default=1, dest='proxy')
args = vars(parser.parse_args())
argsMAC= args['mac']
argsCurrent = args['current']
argsProxy = args['proxy']
...
def captureTest(driver, directory, websites, max):
    ...
    for i in range(0, max):
        ...
        try:
            ...
            captureTestInfo[url] = 'Timeout'
            print("{}: Timeout!").format(websites[i])
        else:
...
if argsCurrent == 0:
    ...
    p = 0
    wifiScanResult = []
    wifiScanResult.append(str(wifi.current()))
...
#STARTING WEB PROXY AND TCPDUMP - 2
...
verify = False
while verify == False:
    confirm = raw_input("Gather Additional Information (/Additional) with G or Finish Run with E: ").lower()
    if confirm in exitO:
        verify = True
    elif confirm in gathering:
...
if argsMAC == 1:
    ...
    if wifi.power() == False:
        print 'Wireless Adapter is Off, turning ON'
        wifi.power(True)
        macAddressChanger(NetworkInterface, MAC)
    else:
        macAddressChanger(NetworkInterface, MAC)
